    ...
    def __call__(self, qpos, image, actions=None, is_pad=None):
        ...
        if actions is not None:  # training time
            if self.use_fast_tokens:
                ...
                logits, is_pad_hat, (mu, logvar) = self.model(qpos, image, env_state, actions, is_pad)

                total_kld, dim_wise_kld, mean_kld = kl_divergence(mu, logvar)
                loss_dict = dict()

                # Cross-entropy loss on token predictions, ignoring pad positions
                # logits: (B, seq_len, vocab_size) → (B*seq_len, vocab_size)
                # targets: (B, seq_len) → (B*seq_len,)
                B, S, V = logits.shape
            ...
            if self.use_fast_tokens:
                # Model outputs logits (1, max_token_len, vocab_size)
                logits, _, (_, _) = self.model(qpos, image, env_state)
                # Argmax to get predicted token IDs
                predicted_tokens = logits.argmax(dim=-1)  # (1, max_token_len)
