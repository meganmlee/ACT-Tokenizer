"""
SkillVAE module from QueST (https://github.com/agiachris/QueST).

Core neural tokenizer: convolutional encoder + VQ/FSQ quantizer +
transformer decoder. Converts continuous action sequences into discrete
token indices and back.

Copied here so ACT-Tokenizer has no runtime dependency on the QueST repo.
"""
...
def get_fsq_level(codebook_size):
    """Map a power-of-2 codebook size to FSQ quantization levels."""
...
class SkillVAE(nn.Module):
    def __init__(self,
        ...
        assert int(np.log2(downsample_factor)) == np.log2(downsample_factor), \
        ...
        if vq_type == 'vq':
            self.vq = VectorQuantize(dim=encoder_dim, codebook_dim=codebook_dim,
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dim_feedforward=4 * encoder_dim, dropout=attn_pdrop,
        ...
        decoder_layer = nn.TransformerDecoderLayer(
            ...
            dim_feedforward=4 * decoder_dim, dropout=attn_pdrop,
            activation='gelu', batch_first=True, norm_first=True)
    ...
    def quantize(self, z):
        if self.vq_type == 'vq':
            ...
            pp = torch.tensor(
                torch.unique(indices).shape[0] / self.vq.codebook_size,
                device=z.device)
        else:
            ...
            pp = torch.tensor(
                torch.unique(indices).shape[0] / self.vq.codebook_size,
                device=z.device)
        pp_sample = torch.tensor(
            np.mean([len(torch.unique(index_seq)) for index_seq in indices]) / z.shape[1],
            device=z.device)
        return codes, indices, pp, pp_sample, commitment_loss

    def decode(self, codes, obs_emb=None):
        x = self.fixed_positional_emb(torch.zeros(
            (codes.shape[0], self.skill_block_size, self.decoder_dim),
...
class CausalConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, stride,
        ...
        self.padding = 0 if no_pad else dilation * (kernel_size - 1)
        self.stride = stride
...
class Conv1dBlock(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, kernel_size, stride,
                 n_groups=4, causal=True, no_pad=False):
        super().__init__()
        if causal:
            conv = CausalConv1d(inp_channels, out_channels, kernel_size,
                                dilation=1, stride=stride, no_pad=no_pad)
        else:
            conv = nn.Conv1d(inp_channels, out_channels, kernel_size,
...
class ResidualTemporalBlock(nn.Module):
    def __init__(self, inp_channels, out_channels, kernel_size=[5, 3],
                 stride=[2, 2], n_groups=8, causal=True, residual=False,
                 pooling_layers=[]):
        super().__init__()
        ...
        for i in range(len(kernel_size)):
            block = Conv1dBlock(
                ...
                out_channels, kernel_size[i], stride[i],
                n_groups=n_groups, causal=causal)
        ...
        if residual:
            if out_channels == inp_channels and stride[0] == 1:
                ...
                self.residual_conv = nn.Conv1d(
                    inp_channels, out_channels, kernel_size=1,
                    stride=sum(stride))
        if pooling_layers:
